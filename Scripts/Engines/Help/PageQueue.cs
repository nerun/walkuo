...
namespace Server.Engines.Help
{
    ...
    public class PageEntry
    {
        ...
        public PageEntry(Mobile sender, string message, PageType type)
        {
            ...
            if (pm != null && pm.SpeechLog != null &&
                Array.IndexOf(SpeechLogAttachment, type) >= 0)
            {
    ...
    public class PageQueue
    {
        ...
        private static void SendEmail(PageEntry entry)
        {
            ...
            foreach (string addr in Email.SpeechLogPageAddresses.Split(new char[] { ',', ';' }))
            {
                if (!string.IsNullOrWhiteSpace(addr))
                {
                    mail.To.Add(new MailboxAddress(addr.Trim(), addr.Trim()));
                }
            }
